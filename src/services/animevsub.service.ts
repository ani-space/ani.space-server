...
import { IAnimevsub } from '../contracts/services/animevsub-service.interface';
...
import { fuzzySearch, getMethodName } from '../utils/tools/functions';

@Injectable()
export class AnimevsubService implements IAnimevsub {
  ...
  constructor(
    ...
    @Inject(AnimeVsub.KEY)
    private readonly animeVsub: ConfigType<typeof AnimeVsub>,
    private readonly eventEmitter: EventEmitter2,
  ) {}

  @OnEvent(SynchronizedAnimeEnum.SYNC_META_INFO)
  public async syncMetaInfo() {
    const list = await this.animeService.getMediaExternalLinkList();
    const mList = cluster(list, 5);

    for await (const m of mList) {
      await Promise.allSettled(
        m.map(async (ml) => {
          const document = await this.generateDocumentFromBrowser(
            `${this.animeVsub.endpoint}${ml.animePath}`,
          );
          ...
          const img = document.querySelector('.Image img')?.getAttribute('src');

          ml.metaInfo = JSON.stringify({
            mainTitle,
            subTitle,
            startDate,
            episodes,
          ...
          await this.animeService.saveMediaExternalLink(ml);
        }),
      );
    }
  }
  ...
  private async generateDocumentFromBrowser(url: string) {
